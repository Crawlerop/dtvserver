...
RunSignal.once("run", (params) => {    
    check_output(params.ffmpeg.replace(/mpeg/g, "probe"), [..."-probesize 48M -loglevel quiet -print_format json -show_error -show_format -show_streams".split(" "), params.src]).then((o) => {
        ...
        ffmp_args.genSingle(params.src, current_rendition, program_streams, params.output_path, params.hls_settings, -1, -1, "", false, params.watermark).then((e) => {
            ...
            const args = (params.realtime ? ["-re", "-loglevel", "repeat+level+warning", "-y", "-probesize", PROBE, "-analyzeduration", "0", "-stats_period", "2", "-progress", "-"] : ["-loglevel", "repeat+level+warning", "-y", "-probesize", PROBE, "-analyzeduration", "0", "-stats_period", "2", "-progress", "-"]).concat(params.src.startsWith("rtsp") ? ["-rtsp_transport", "tcp"] : params.src.startsWith("http") ? ["-rw_timeout", "30000000"] : params.src.endsWith(".m3u8") ? ["-live_start_index", "-1"] : []).concat(e)
