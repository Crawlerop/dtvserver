            ...
            */
            ...
            if (Object.keys(params.dtv_udp_out).indexOf(dtv_key) !== -1) {
                if (params.use_protocol === "tcp") {
                    ...
                    tsp_args.push(`tsp -P zap ${channel.id} | ${params.ffmpeg} -copyts -i - -vcodec copy -acodec copy -copyinkf -loglevel error -f mpegts tcp://${params.dtv_udp_out[dtv_key]}?send_buffer_size=1316`)
                    ...
                    //tsp_args.push(`tsp --realtime --buffer-size-mb 8 --max-flushed-packets 7 --max-output-packets 7 --max-input-packets 7 -P zap ${channel.id} -O ip ${params.dtv_udp_out[dtv_key]}`)
                    tsp_args.push(`tsp -P zap ${channel.id} | ${params.ffmpeg} -copyts -i - -vcodec copy -acodec copy -copyinkf -loglevel error -f mpegts -flush_packets 0 udp://${params.dtv_udp_out[dtv_key]}?pkt_size=1316`)
                } else {
