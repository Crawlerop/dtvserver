...
RunSignal.once("run", async (params) => {    
   try {
    ...
    var tsp_args = `--buffer-size-mb ${params.buffer_size} --receive-timeout 10000 --realtime -I dvb --signal-timeout 10 --guard-interval auto --adapter ${params.tuner} --delivery-system ${params.system} --demux-buffer-size 4194304 --frequency ${params.frequency}000000 --transmission-mode auto --spectral-inversion off`.split(" ")
    ...
    */
    ...
    for (let i = 0; i<params.channels.length; i++) {
        ...
        const tsp_fork_prm = ["-y", "-loglevel", "repeat+level+error"].concat(await ffmp_args.genSingle(params.dtv_use_fork ? "-" : `unix:${LS_SOCKET}`, current_rendition, streams, out_folder, params.hls_settings, channel.video.id, channel.audio ? channel.audio.id : -1, audio_filters, passed_params.dtv_use_fork ? true : false, used_watermark))
