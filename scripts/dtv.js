            ...
            */
            ...
            if (RESTART_EACH_STREAMS) {
                ...
                if (!REPEAT_DETECT_STALLS) {
                    ...
                    tsp_args.push(`tsp -P zap ${channel.id} | node ${path.join(__dirname, "/cmds")}/repeat4.js "${channel.name}" ${channel.video.fps >= 30 ? (channel.video.fps / 2) : channel.video.fps} $PPID ${params.ffmpeg} -stats_period 2 -progress - -nostats ${tsp_fork_prm.join(" ")}`)
                }
