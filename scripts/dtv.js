...
RunSignal.once("run", async (params) => {    
   try {
    ...
    var tsp_args = `-I dvb --demux-buffer-size 33554432 --signal-timeout 10 --guard-interval auto --receive-timeout 10000 --adapter ${params.tuner} --delivery-system DVB-T2 --frequency ${params.frequency}000000 --transmission-mode auto --spectral-inversion off`.split(" ")
    var folders = []
    var ad_param;
    ...
    for (let i = 0; i<params.channels.length; i++) {
        ...
        tsp_args.push("-P")
        tsp_args.push("fork")        
        tsp_args.push(`tsresync -c - | tsp | node ${path.join(__dirname, "/cmds")}/repeat.js "${channel.name}" ${params.ffmpeg} -progress - -nostats ${tsp_fork_prm.join(" ")}`)
    }
    tsp_args.push("-O")
    tsp_args.push("drop")
