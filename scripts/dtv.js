...
RunSignal.once("run", async (params) => {    
    ...
    */
    ...
    for (let i = 0; i<params.channels.length; i++) {
        ...
        const tsp_fork_prm = ["-y", "-loglevel", "repeat+level+error", "-probesize", "32"].concat(await ffmp_args.genSingle(params.dtv_use_fork ? "-" : `unix:${LS_SOCKET}`, current_rendition, streams, out_folder, params.hls_settings, -1, channel.audio ? channel.audio.id : -1, audio_filters, passed_params.dtv_use_fork ? true : false, used_watermark))
