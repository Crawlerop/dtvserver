            ...
            */
            ...
            if (RESTART_EACH_STREAMS) {
                ...
                if (!REPEAT_DETECT_STALLS) {
                    tsp_args.push(`tsp --buffer-size-mb ${params.buffer_size*2} -P zap ${channel.id} | node ${path.join(__dirname, "/cmds")}/repeat2.js "${channel.name}" '${params.ffmpeg} ${tsp_fork_prm.join(" ")}'`)
                } else {
                    tsp_args.push(`tsp --buffer-size-mb ${params.buffer_size*2} -P zap ${channel.id} | node ${path.join(__dirname, "/cmds")}/repeat4.js "${channel.name}" ${channel.video.fps >= 30 ? (channel.video.fps / 2) : channel.video.fps} ${params.ffmpeg} -stats_period 2 -progress - -nostats ${tsp_fork_prm.join(" ")}`)
                }
