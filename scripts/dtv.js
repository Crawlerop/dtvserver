...
RunSignal.once("run", async (params) => {    
   try {
    ...
    var tsp_args = `--buffer-size-mb 384 --realtime -I dvb --signal-timeout 10 --guard-interval auto --receive-timeout 10000 --adapter ${params.tuner} --delivery-system DVB-T2 --frequency ${params.frequency}000000 --transmission-mode auto --spectral-inversion off`.split(" ")
    ...
    */
    ...
    for (let i = 0; i<params.channels.length; i++) {
        ...
        const tsp_fork_prm = ["-re", "-y", "-loglevel", channel.id == 5 ? "error": "quiet"].concat(await ffmp_args.genSingle(params.dtv_use_fork ? "-" : `unix:${LS_SOCKET}`, current_rendition, streams, out_folder, params.hls_settings, -1, -1, audio_filters, passed_params.dtv_use_fork ? true : false))
